      ...
      tabsetPanel(id = "sidebar_tabs",
                  ...
                  tabPanel("Injections",
                           ...
                           numericInput("target_po4", 
                                        "Target PO4-P Increase (ppb)", 
                                        value = 70, 
                                        min = 0),
                           numericInput("target_no3", 
                                        "Target NO3-N Increase (ppb)", 
                                        value = 1000, 
                                        min = 0),
                  ...
                  tabPanel("Settings",
                           ...
                           numericInput("po4_stock_conc", 
                                        "PO4-P Stock Solution Concentration (mg/L as P)", 
                                        value = 1000000, 
                                        min = 0,
                                        step = 1000),
                           numericInput("no3_stock_conc", 
                                        "NO3-N Stock Solution Concentration (mg/L as N)", 
                                        value = 1000000, 
                                        min = 0,
                                        step = 1000),
  ...
  observeEvent(input$calculate_injections, {
    ...
    if (length(excess_indices) > 0) {
      ...
      if (n_variable > 0) {
        ...
        # Find the index for each quantile
        variable_indices <- integer(length(target_quantiles))
        for (q_i in seq_along(target_quantiles)) {
          # Find first index where CDF >= quantile
          idx_in_window <- which(cum_conc >= target_quantiles[q_i])[1]
      ...
      while (length(sample_indices) < n_samples && length(sample_indices) < count_cond) {
        # Find gap with largest concentration area not yet sampled
        gaps <- diff(sample_indices)
        gap_conc <- numeric(length(gaps))
        for (g in seq_along(gaps)) {
          seg <- sample_indices[g]:sample_indices[g + 1]
          gap_conc[g] <- sum(pmax(excess_conductivity[seg], 0))
        }
        best_gap_idx <- which.max(gap_conc)
        new_idx <- round((sample_indices[best_gap_idx] + sample_indices[best_gap_idx + 1]) / 2)
        if (new_idx %in% sample_indices) break
  ...
  output$tracer_mass_results <- renderUI({
    ...
    div(
      ...
      p(strong(sprintf("  → SOLUTION to add       = %.4f g", calc_results$mass_rhodamine$g_solution)), 
        style = "margin-left: 20px;"),
      br(),
      p(strong(sprintf("PO4-P (Target: %.2f ppb as P):", input$target_po4))),
      p(strong(sprintf("  → Add %.2f mL of stock solution (%.0f mg/L as P)", 
                       calc_results$mass_po4$mL, 
                       calc_results$mass_po4$stock_conc_mgL)), 
        style = "margin-left: 20px;"),
      br(),
      p(strong(sprintf("NO3-N (Target: %.2f ppb as N):", input$target_no3))),
      p(strong(sprintf("  → Add %.2f mL of stock solution (%.0f mg/L as N)", 
    ...
    content = function(file) {
      ...
      if (method == "injections" && !is.null(calc_results$mass_arabinose)) {
        params <- c(params,
                     ...
                     "Target PO4-P (ppb as P)", "PO4-P Stock Concentration (mg/L as P)",
                     "Required PO4-P Stock Volume (mL)", "Required PO4-P Mass (g as P)", "",
                     "Target NO3-N (ppb as N)", "NO3-N Stock Concentration (mg/L as N)",
                     "Required NO3-N Stock Volume (mL)", "Required NO3-N Mass (g as N)", "")
        values <- c(values,
